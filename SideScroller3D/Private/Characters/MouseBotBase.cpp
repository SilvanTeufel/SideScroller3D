			...
			if(CurrentEnemy)
			{
				...
				if(!AttackBottom)
				{
					...
					if(AttackIterationTime >= 1.f)
					{
						...
						CurrentEnemy->SetHealth(CurrentEnemy->Attributes->GetHealth()-AttackDamage);
					}
	...
	case BotData::Heal_Bot:
		{
			...
			if(CurrentFriend)
			{
				...
				FVector EDirection = UKismetMathLibrary::GetDirectionUnitVector(CurrentLocation, EnemyOrigin);
				//UE_LOG(LogTemp, Warning, TEXT("Enemy Origin/BoxExtent %s // %s"), *EnemyOrigin.ToString(), *EnemyBoxExtent.ToString() );
				...
				if(!HealBottom)
				{
					...
					{
						HealIterationTime = 0;
						//CurrentFriend->IsAttacked(this);
						
						CurrentFriend->SetHealth(CurrentFriend->Attributes->GetHealth()+HealPoints);
						if(CurrentFriend->Attributes->GetHealth() >= CurrentFriend->Attributes->GetMaxHealth())
						{
							SetAnimState(BotData::Run_Bot);
						}
					}
